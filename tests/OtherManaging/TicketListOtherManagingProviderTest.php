...
/**
 * Class TicketListOtherManagingProviderTest
 */
class TicketListOtherManagingProviderTest extends TestCase
{
     ...
     */
    public function testTicketListShouldNotFailsWhenFiltersAreEmpty(): void
    {
        $transport = $this->createMock(TransportInterface::class);
        $transport->expects($this->once())
            ->method('get')
            ->with('/3.0/admin/tickets', [])
            ->willReturn(
                [
                    'status' => 1,
                    'tickets' => static::$attributes,
                    'pagination' => ['per_page' => 1, 'total_count' => 1, 'page' => 1],
                ]
            );

        $otherManagingProvider = new OtherManagingProvider($transport);
        $otherManagingProvider->ticketList([]);
    }
     ...
     * @throws \Affise\Sdk\Exception\TransportException
     */
    public function testTicketListWhenFiltersAreSet(): void
    {
        $transport = $this->createMock(TransportInterface::class);
        ...
        $transport->expects($this->once())
            ->method('get')
            ->with('/3.0/admin/tickets', $filters)
            ->willThrowException(new RuntimeException());

        $this->expectException(RuntimeException::class);

        $otherManagingProvider = new OtherManagingProvider($transport);
        $otherManagingProvider->ticketList($filters);
    }
     ...
     * @throws \Affise\Sdk\Exception\TransportException
     */
    public function testTicketListResponse(): void
    {
        $transport = $this->createMock(TransportInterface::class);
        ...
        $otherManagingProvider = new OtherManagingProvider($transport);
        $response = $otherManagingProvider->ticketList();

        $expectedData = array_map(fn(array $a) => new TicketDto($a), $attributes);
     ...
     */
    public function testTicketListFailsWhenTransportLayerThrowsException(string $exceptionClass): void
    {
